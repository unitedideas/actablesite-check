...
export const crawlerWatchPitch = "Monitor homepage indexability and AI crawler policy every 15 minutes; confirmed changes arrive by email for $9/month.";
...
export const homepageIndexabilityLimit = "Homepage checks inspect the returned HTTP response, raw HTML, and response headers without running browser JavaScript. They do not prove crawling, indexing, ranking, traffic, or revenue.";
...
function normalizedDirectives(values) {
  return [...new Set(values.flatMap((value) => String(value || "").toLowerCase().split(",")).map((value) => value.trim()).filter(Boolean))].sort();
}

export function parseHomepageIndexability({ body = "", finalUrl, status, xRobotsTag = "" }) {
  const metaRobots = [];
  let canonicalUrl = null;
  for (const match of body.matchAll(/<(?:meta|link)\b[^>]*>/gi)) {
    const tag = match[0];
    const name = htmlAttribute(tag, "name")?.toLowerCase();
    if (tag.toLowerCase().startsWith("<meta") && name === "robots") {
      const content = htmlAttribute(tag, "content");
      if (content) metaRobots.push(content);
    }
    const rel = htmlAttribute(tag, "rel")?.toLowerCase().split(/\s+/) || [];
    if (!canonicalUrl && tag.toLowerCase().startsWith("<link") && rel.includes("canonical")) {
  ...
  const metaRobotsDirectives = normalizedDirectives(metaRobots);
  const xRobotsTagDirectives = normalizedDirectives([xRobotsTag]);
  return {
    finalUrl,
    responseStatus: status,
    state: status >= 200 && status < 400 ? "reachable" : "restricted",
    metaRobots: metaRobotsDirectives,
    xRobotsTag: xRobotsTagDirectives,
    canonicalUrl,
    noindex: [...metaRobotsDirectives, ...xRobotsTagDirectives].some((directive) => /(^|:)\s*noindex\b/.test(directive)),
  };
}

export async function checkHomepageIndexability(input, { fetchImpl = fetch, timeoutMs = 10000 } = {}) {
  const robotsUrl = normalizePublicUrl(input);
  const siteUrl = new URL(robotsUrl.origin);
  try {
    const response = await fetchImpl(siteUrl, {
      headers: { accept: "text/html,*/*;q=0.5", "user-agent": "actablesite-check/1.5.0" },
      redirect: "follow",
      signal: AbortSignal.timeout(timeoutMs),
    });
    const body = await response.text();
    if (body.length > 1_000_000) throw new Error("homepage response exceeds the 1 MB safety limit.");
    return {
      site: siteUrl.origin,
      checkedAt: new Date().toISOString(),
      ...parseHomepageIndexability({
        body,
        finalUrl: response.url || siteUrl.toString(),
        status: response.status,
        xRobotsTag: response.headers.get("x-robots-tag") || "",
      }),
      limits: homepageIndexabilityLimit,
    };
  } catch (error) {
    if (error instanceof Error && /1 MB safety limit/.test(error.message)) throw error;
    return {
      site: siteUrl.origin,
      checkedAt: new Date().toISOString(),
      finalUrl: siteUrl.toString(),
      responseStatus: null,
      state: "unavailable",
      metaRobots: [],
      xRobotsTag: [],
...
export async function checkWebsite(input, { fetchImpl = fetch, timeoutMs = 10000 } = {}) {
  ...
  const response = await fetchImpl(robotsUrl, {
    headers: { accept: "text/plain,*/*;q=0.5", "user-agent": "actablesite-check/1.5.0" },
    redirect: "follow",
    signal: AbortSignal.timeout(timeoutMs),
  });
  const body = await response.text();
